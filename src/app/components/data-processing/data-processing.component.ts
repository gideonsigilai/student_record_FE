...
import { LucideAngularModule, FileSpreadsheet, Upload, Loader2, CheckCircle2, AlertCircle, Timer, X } from 'lucide-angular';
    ...
    <div class="card">
      <div class="flex items-center gap-3 mb-6">
        <div class="p-3 bg-purple-500/20 rounded-xl">
          <lucide-icon name="file-spreadsheet" class="w-6 h-6 text-purple-400"></lucide-icon>
        </div>
        <div>
          <h2 class="text-xl font-bold text-white">Data Processing</h2>
          <p class="text-slate-400 text-sm">Task B: Convert Excel to CSV</p>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          @if (!selectedFile()) {
            <div class="space-y-3">
              ...
              <div>
                <p class="text-white font-medium">Drop your Excel file here</p>
                <p class="text-slate-400 text-sm mt-1">or click to browse</p>
              </div>
              <p class="text-slate-500 text-xs">Supports: .xlsx, .xls</p>
            </div>
          } @else {
            <div class="space-y-2">
        ...
        <button 
          (click)="process()"
          [disabled]="!selectedFile() || studentService.isProcessing()"
          class="btn-primary w-full flex items-center justify-center gap-2"
        >
          @if (studentService.isProcessing()) {
            <lucide-icon name="loader-2" class="w-5 h-5 animate-spin"></lucide-icon>
            <span>Processing...</span>
          } @else {
            <lucide-icon name="file-spreadsheet" class="w-5 h-5"></lucide-icon>
            <span>Process Excel File</span>
          }
        </button>
...
export class DataProcessingComponent {
    ...
    process() {
        ...
        this.studentService.processFileWithDownload(file).subscribe({
            next: (blob: Blob) => {
                const endTime = Date.now();
                const timeInMillis = endTime - startTime;
                
                this.studentService.isProcessing.set(false);
                this.isSuccess.set(true);
                this.status.set('Excel file successfully converted to CSV (Score + 10 applied)');
                this.performanceTime.set(timeInMillis);
                this.studentService.lastProcessTime.set(timeInMillis);
                
                // Auto-download the processed CSV file
                this.downloadFile(blob, 'processed_students.csv');
            },
            error: (error) => {
                this.studentService.isProcessing.set(false);
